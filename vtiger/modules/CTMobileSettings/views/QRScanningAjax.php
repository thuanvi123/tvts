...
Class CTMobileSettings_QRScanningAjax_View extends Vtiger_IndexAjax_View {
    ...
    public function process(Vtiger_Request $request) {
        ...
        $rs=$adb->pquery("SELECT * FROM `ctmobile_qrscanning_modules` WHERE module=?",array($moduleName));
        if($adb->num_rows($rs) >0) {
            $viewer->assign('ACTIVE', $adb->query_result($rs,0,'active'));
            // Get selected fields
            $rs_field=$adb->pquery("SELECT * FROM `ctmobile_qrscanning_fields` WHERE module=?",array($moduleName));
        ...
        $modulefield = '';
        $newQuery = $adb->pquery("SELECT * FROM ctmobile_qrscanning WHERE module = ?",array($moduleName));
        if($adb->num_rows($newQuery) > 0){
            $modulefield = $adb->query_result($newQuery,0,'modulefield');
        }
        $viewer->assign('SELECTED_FIELDS', $selectedFields);
        $viewer->assign('MODULE_FIELD', $modulefield);
        $viewer->assign('USER_MODEL', Users_Record_Model::getCurrentUserModel());
        $viewer->assign('SOURCE_MODULE', $moduleName);
        echo $viewer->view('QRSelectedFields.tpl', $module, true);
    }
}
