...
Class CTMobileSettings_VcardFieldsAjax_View extends Vtiger_IndexAjax_View {

    public function process(Vtiger_Request $request) {
        global $adb;
        $viewer = $this->getViewer ($request);
        $moduleName = $request->get('vcard_module');
        ...
        if($adb->num_rows($rs) >0) {
            ...
            // Get selected fields
            $rs_field=$adb->pquery("SELECT * FROM `ctmobile_vcard_fields` WHERE module=?",array($moduleName));
        ...
        echo $viewer->view('VcardSelectedFields.tpl', $module, true);
    }
}
