...
class CTTimeTracker_List_View extends Vtiger_Index_View {
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
		...
		/*print_r($this->listViewEntries);
		die;*/
		foreach($this->listViewEntries as $key => $value){
			
			$checkinlocation = $this->getCheckInLocation($key);
			$checkoutlocation = $this->getCheckOutLocation($key);

			$DetaiViewurl = 'index.php?module='.$moduleName.'&view=Detail&record='.$key;
			$createdtime = Vtiger_Util_Helper::formatDateDiffInStrings($value->get('createdtime'));
			if($checkinlocation != ',' && $checkinlocation != ''){
				$temp1 = explode(',',$checkinlocation);
				$latitude = $temp1[0];
				$longitude = $temp1[1];
				if((double)$latitude != 0 && (double)$longitude != 0){
					$label = $value->get('tracking_title');
					$action = 'Check-in Point';
					$entitylabel = '<div id="bodyContent"><p>'.$action.'</p><p><a href="'.$DetaiViewurl.'" target="_blank"><b>'.$label.' </b></a><p>'.$createdtime.'</p></div>';
					$data['marker'][] = array('lat'=>(double)$latitude,'lng'=>(double)$longitude,'label'=>$entitylabel,'recordId' => $key);
				}
			}

			if($checkoutlocation != ',' && $checkoutlocation != ''){
				$temp1 = explode(',',$checkoutlocation);
				$latitude = $temp1[0];
				$longitude = $temp1[1];
				if((double)$latitude != 0 && (double)$longitude != 0){
					$label = $value->get('tracking_title');
					$action = 'Check-out Point';
					$entitylabel = '<div id="bodyContent"><p>'.$action.'</p><p><a href="'.$DetaiViewurl.'" target="_blank"><b>'.$label.' </b></a><p>'.$createdtime.'</p></div>';
					$data['marker'][] = array('lat'=>(double)$latitude,'lng'=>(double)$longitude,'label'=>$entitylabel,'recordId' => $key);
				}
			}
		...
		/*print_r($data);
		die;*/
		$viewer->assign('MAPPOLILINE',json_encode($data['marker'])); 
		/*map code end*/
	...
	function getCheckInLocation($recordId){
		$check_in_location = "";
		if($recordId){
			$recordModel = CTTimeTracker_Record_Model::getInstanceById($recordId,'CTTimeTracker');
			$check_in_location = $recordModel->get('check_in_location');
		}
		return $check_in_location;
	}

	function getCheckOutLocation($recordId){
		$check_out_location = "";
		if($recordId){
			$recordModel = CTTimeTracker_Record_Model::getInstanceById($recordId,'CTTimeTracker');
