...
class CTMobile_WS_GetRouteEditDetails extends CTMobile_WS_Controller {
	...
	function process(CTMobile_API_Request $request) {
		...
		$date_query = 'SELECT * FROM vtiger_ctrouteplanning INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_ctrouteplanning.ctrouteplanningid WHERE ctrouteplanningid = ?';
		$ctroute_result = $adb->pquery($date_query,array($recordId));
		$ctroutename = $adb->query_result($ctroute_result,0,'ctroutename');
		...
		$smownerid = $adb->query_result($ctroute_result,0,'smownerid');

		$userRecordModel = Vtiger_Record_Model::getInstanceById($smownerid,'Users');
		if(!empty($userRecordModel->get('user_name'))){
			$assigned_user_id = array('value'=>vtws_getWebserviceEntityId('Users', $smownerid),'label'=>html_entity_decode($userRecordModel->get('first_name').' '.$userRecordModel->get('last_name'),ENT_QUOTES,$default_charset));
		}else{
			$gquery = "SELECT groupname FROM vtiger_groups WHERE groupid = ?";
			$groupResults = $adb->pquery($gquery,array($smownerid));
			$assigned_user_id = array('value'=>vtws_getWebserviceEntityId('Groups', $smownerid),'label'=>html_entity_decode($adb->query_result($groupResults,0,'groupname'),ENT_QUOTES,$default_charset));
		}
		...
		if(count($ctroute_realtedtoList) == 0) {
			...
			$response->setResult(array('ctroutename'=>$ctroutename,'records'=>$ctroute_realtedtoList,'ctroute_date'=>$ctroute_date,'assigned_user_id'=>$assigned_user_id,'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>$message,'isLast'=>$isLast));
		} else {
			$response->setResult(array('ctroutename'=>$ctroutename,'records'=>$ctroute_realtedtoList,'ctroute_date'=>$ctroute_date,'assigned_user_id'=>$assigned_user_id,'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>'','isLast'=>$isLast));
		}
		return $response;
	}
