...
class CTMobile_WS_GetEventList extends CTMobile_WS_Controller {
	...
	function process(CTMobile_API_Request $request) {
		...
		if (empty($date)) {
			$message = vtranslate('Date cannot be empty!','CTMobile');
	   ...
	   usort($recentEvent_data, function ($a, $b) use(&$name){
		...
		if($adb->num_rows($query) == 0){
			$response->setResult(array('GetEventList'=>[],'module'=>'Events','code'=>404,'message'=>vtranslate('No Upcoming Event','CTMobile')));
		} else {
