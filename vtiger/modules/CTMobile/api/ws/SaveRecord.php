	...
	function process(CTMobile_API_Request $request) {
		...
		try {
			...
			if($module == 'ModComments'){
				$this->recordValues['commentcontent'] = decode_html(decode_html($request->get('commentcontent')));
			}
			...
			if(in_array($module,$lineItemsModules)){
				...
				if($charges){
					...
					$result = $adb->pquery($insertShipQuery,array($ID[1],json_encode($charges)));
				}
				if($module == 'Invoice'){
					...
					$result = $adb->pquery($query,array($subtotal,$total,$s_h_amount,$s_h_percent,$pre_tax_total,$balance,$conversionRate,$ID[1]));
					...
					$result = $adb->pquery($query,array($subtotal,$total,$s_h_amount,$s_h_percent,$pre_tax_total,$conversionRate,$ID[1]));
				}
			...
			if($module == 'Events' || $module == 'Calendar'){
		        ...
		        if(!empty($selectUsers)){
		            ...
		            $recordModel = Vtiger_Record_Model::getInstanceById($recordId[1],'Events');
			...
			$result = array('id'=>$this->recordValues['id'],'recordLabel'=>$recordLabel,'module'=>$module,'message'=>$message);
